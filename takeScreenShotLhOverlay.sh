...
scriptDir="/data/joy/BBL/studies/STOP_PD/STOP_PD_baselineNMF/";
...
# path for standard figures - the figures are going to be saved in the dropbox folder
fig_path="${output_path}/Figures_Lh/tkViews"
if [ ! -d "${fig_path}" ]; then	
	mkdir -p ${fig_path}
fi
	
# path for figures with color bar
fig_color_path="${output_path}/Figures_Lh/ColorBar"
if [ ! -d "${fig_color_path}" ]; then	
	mkdir -p ${fig_color_path}
fi

index=0	
for f in $(find ${overlay} -name "*lh.mgh")
do
	mgh_name=$(echo ${f##*/})
	...
	then
		...
		tksurfer fsaverage lh inflated -overlay ${overlay}/${mgh_name} -fminmax ${min} ${max} -fmid ${mid} -tcl "${fig_path}/${new_name}_shotscript.tcl"
	fi
